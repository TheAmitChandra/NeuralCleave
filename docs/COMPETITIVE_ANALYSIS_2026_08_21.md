...
## 8. Follow-up (same day) — the CLI cross-process gap flagged in §7 is now fixed

§7's last bullet flagged `neuralcleave approvals pending/approve/deny` as a real, documented-but-unfixed limitation and suggested "make the CLI proxy through the REST API when a gateway is reachable" for a future round. That fix landed the same day in a follow-up PR: `_try_gateway_json()` now attempts `GET`/`POST /api/v1/approvals/...` against the configured `[gateway]` bind/port first (short 2s timeout, `X-API-Key` header included when configured), and only falls back to the CLI process's own in-memory `ApprovalQueue` — printing a note when it does — if nothing answers there. A real HTTP error from a gateway that IS running (e.g. an unknown approval id) still surfaces as a `ClickException` rather than being treated as "no gateway," so a genuine 404 doesn't get silently swallowed into an unhelpful local-fallback message. `allowlist add/list/remove` needed no change — already correctly cross-process via SQLite.
