# NeuralCleave vs OpenClaw — Full Capability Analysis

> **July 2026 · NeuralCleave v2.1.0**  
...
| Metric | Count |
|---|---|
| NeuralCleave leads | **13** categories |
| Parity | **20** categories |
| OpenClaw leads | **0** categories |
| NeuralCleave missing entirely | **0** capabilities |
| Channels — NeuralCleave | **32** |
...
- The **Tauri installer (.exe)** bundles `neuralcleave-backend.exe` (PyInstaller) as a sidecar
...
- `pip install neuralcleave` is the **developer path** and should not open any desktop UI automatically
- The `NeuralCleave-desktop` console script added to `pyproject.toml` is correct — it is the binary Tauri invokes
...
NeuralCleave has 32 production-ready adapters, each with a normalized `InboundMessage` interface.  
OpenClaw ships 29+ channels. **NeuralCleave leads on channel count.**
...
### OpenClaw has, NeuralCleave does NOT ❌
...
### NeuralCleave — Python / FastAPI

- FastAPI + uvicorn gateway, default port 7432
- Structured TOML config (`~/.NeuralCleave/config.toml`); typed dataclasses; `ENV:` secret resolution
...
- Plugin entry-points via `importlib.metadata` (PEP 451); `NeuralCleave-sdk` for plugin authors
...
| File system | read / write / append / list / delete / move / copy / mkdir / stat / search; `~/NeuralCleave_files/` default + `allowed_paths` for full host access | Full host filesystem access (or Docker/SSH sandbox) | **Parity** |
...
| REST API key auth | ✅ Optional `X-API-Key` middleware on all `/api/*` routes; disabled by default (empty `gateway.api_key`); exempt: `/health`, `/ws/*`; resolved via `ENV:` config | Not documented | **CF leads** |

---

## Where NeuralCleave Leads — 12 Clear Advantages
...
NeuralCleave now ships 32 production-ready channel adapters — Bluesky (AT Protocol polling), Viber (REST + HMAC-SHA256 webhook), and XMPP/Jabber (slixmpp MUC + 1:1 chat) added in this release. OpenClaw documents "29+" channels. **No equivalent channel count advantage exists for OpenClaw.**

### 11. Progressive Web App (Mobile Companion)
NeuralCleave ships a PWA served directly from the FastAPI gateway at `/app`. The W3C-compliant manifest (`/manifest.json`) triggers the browser's install prompt on iOS and Android, making NeuralCleave installable as a home-screen app with no app store involvement. The embedded Service Worker provides cache-first offline access for the shell and network-first pass-through for the API and WebSocket. Web Push is wired via a VAPID key endpoint and a file-backed `PushManager` subscription store. The chat UI connects over WebSocket using the existing `hello`/`message_chunk`/`message_done` protocol so it is identical to the desktop client functionally. OpenClaw ships separate native iOS and Android beta apps requiring separate installs; the PWA approach gives NeuralCleave equivalent mobile reach at zero native-code cost.

### 12. Typed Plugin SDK
`NeuralCleave-sdk` exposes clean ABC interfaces (`Plugin`, `Tool`, `ChannelAdapter`) with `importlib.metadata` PEP 451 entry-point discovery. Plugin authors import only the SDK, never the gateway — better isolation and upgrade safety than OpenClaw's markdown-based `TOOLS.md` system.
...
|---|---|---|
| ~~Tauri `main.rs` sidecar spawn — complete `src-tauri/src/main.rs`~~ | ✅ **Done** — sidecar spawn in `lib.rs`, `bundle_backend.ps1` + `neuralcleave-backend.spec` shipped in PR #41 | — |
...
| ~~LINE / Feishu / Zalo channels~~ | ✅ **Done** — `LineAdapter` (PR #47), `FeishuAdapter` (PR #48), `ZaloAdapter` (PR #53) | — |
...
| Dimension | NeuralCleave | OpenClaw | Verdict |
|---|---|---|---|
...
| Skill ecosystem | ✅ NeuralCleave Hub marketplace + PackageScanner safety scanner; `HubRegistry`; 8 REST + 9 CLI commands | 3,500+ ClawHub skills | **Parity** |
...
| Date | Change |
|---|---|
| 2026-07-21 | **Docs site overhaul — `feat/docs-site-overhaul`** — Replaced the marketing landing page at `docs-site/` with a full technical documentation site hosted on GitHub Pages at `docs.neuralcleave.com`. Deleted dead files: `styles.css`, `charts.js`, `script.js`. New assets: `deploy-docs.yml` (GitHub Pages workflow via `actions/upload-pages-artifact@v3` + `actions/deploy-pages@v4`), `CNAME` (`docs.neuralcleave.com`), `css/docs.css` (complete dark/light design system, all components, no CDN), `js/docs.js` (theme toggle, client-side search with 100+ index entries, TOC scroll-spy, copy buttons). 14 doc pages written: `index.html` (stats strip: 32 channels / 13 providers / 41 endpoints / 5,064 tests), `getting-started.html`, `architecture.html`, `memory.html`, `llm-providers.html` (all 13 providers with TOML examples), `channels.html` (all 32 adapters with TOML config), `configuration.html`, `cli.html` (all subcommands), `api.html` (all 41 REST endpoints + 2 WebSocket), `plugins.html` (Tool/Plugin/ChannelAdapter SDK + Hub + PackageScanner), `voice.html` (Whisper STT / 3-tier TTS / wake word / voice cloning), `observability.html` (13 Prometheus metrics table + ContextLogger), `deployment.html` (Docker / docker-compose / Railway / Render / cloud CLI), `testing.html` (5,064 tests, coverage by module, writing tests, CI jobs). DNS: add CNAME record `docs → theamitchandra.github.io` at domain registrar to activate `docs.neuralcleave.com`. |
| 2026-07-16 | **Gateway auth + config depth + test coverage** — `feat/gateway-auth-and-test-depth`. Added 8 missing LLM provider API-key fields to `ModelsConfig` (`mistral_api_key`, `xai_api_key`, `cohere_api_key`, `moonshot_api_key`, `zhipuai_api_key`, `dashscope_api_key`, `qianfan_api_key`, `ark_api_key`) + `api_key` to `GatewayConfig`; all support `ENV:VAR_NAME` resolution via `resolve_secret()`. Optional `X-API-Key` HTTP middleware in `create_app()` enforces auth on all `/api/*` routes when `gateway.api_key` is non-empty; `/health` and `/ws/*` are always exempt. 122 new tests: 21 auth middleware (disabled / correct key / wrong key / exempt routes / config parsing), 35 extended config (new provider key defaults + TOML parsing + ENV: resolution + gateway api_key lifecycle), 25 update_checker edge cases (parse_version, is_newer, network failures, malformed responses, httpx ImportError), 41 orchestrator routing (NoEligibleNodeError, NodeNotFoundError, fallback node, channel routing via glob, keyword routing, round-robin tie-breaking, disable/enable, re-registration stat reset, route() AgentResult contract, get_node_namespaces). Security > REST API key auth: Not documented → **NeuralCleave leads**. Total tests: 5064 (was 4942). |
| 2026-07-16 | **Full codebase rename: CortexFlow → NeuralCleave** — PR #74. Package `cortexflow-ai` → `neuralcleave`; directory `cortexflow_ai/` → `neuralcleave/`; CLI entry point `cortex`/`cortexflow-desktop` → `neuralcleave`/`neuralcleave-desktop`; config path `~/.cortexflow/` → `~/.neuralcleave/`; Tauri identifier `com.neuralcleave.desktop`; sidecar `neuralcleave-backend`; 4942 tests pass. |
| 2026-07-16 | **Frontend bug fixes + cross-platform CI** — Fixed Canvas page calling non-existent `/api/v1/canvas/snapshot` → now calls `/api/v1/canvas/state` with correct `{available, blocks, count}` shape; `BlockCard` renders all 7 block types (text, markdown, code, image, table, chart, html). Fixed Skills page calling non-existent `/api/v1/skills` → now calls `/api/v1/hub/packages` with correct `{available, packages}` shape; adds tag chips + PackageScanner security badge. Added `NEURALCLEAVE_SKIP_BUNDLE` env-var skip logic to `bundle_backend_dispatch.js` (avoids running PyInstaller twice in CI). Updated `.github/workflows/build-tauri.yml` with `actions/setup-python@v5` + `pip install pyinstaller && pip install -e .` + platform-specific bundle step so `npx tauri build` works on all three runners (Windows, macOS, Linux). Added 25 frontend-to-backend contract tests (`test_frontend_page_endpoints.py`) verifying every page calls the correct endpoint with the right response shape. |
| 2026-07-15 | **Per-node memory isolation shipped** — `neuralcleave/orchestrator/memory.py` added (PR #69). `MemoryEntry` dataclass (key, value, namespace, created_at, tags; `to_dict()`). `MemoryNamespaceStore`: ordered LRU key-value store per namespace; configurable `max_entries` (default 1000); `put` (LRU evict on overflow), `get`, `delete`, `search` (case-insensitive key+value substring, optional tag filter), `list_by_tag`, `clear`, `all_entries`, `all_keys`, `stats`. `MemoryNamespaceManager`: lazy registry of stores; `namespace()` creates on first access; `put/get/delete/search` convenience pass-throughs; `clear_namespace/drop_namespace/list_namespaces/global_stats/namespace_stats`. `AgentNodeConfig` gains `memory_namespace: str = ""` field + `effective_memory_namespace` property (falls back to node name) + `from_dict()` classmethod; `to_dict()` includes both fields. `AgentOrchestrator.__init__` accepts optional `memory_manager` injection; `route()` includes `memory_namespace` in result metadata; new `get_node_namespaces()` and `memory_for_node()` methods. Three new REST endpoints: `GET /api/v1/orchestrator/nodes/{name}/memory`, `DELETE /api/v1/orchestrator/nodes/{name}/memory`, `GET /api/v1/orchestrator/namespaces`. 156 new tests (76 memory unit + 44 node namespace + 36 HTTP routes). Advantages heading: 11 → **12**. |
| 2026-07-08 | **Shell execution gap closed** — `ShellTool` added (PR #34). `shell=False` always; allowlist of 30+ safe programs; sandbox constrained to `~/neuralcleave_files/`; sensitive env vars stripped; 50 KB output cap; hard timeout; UTF-8 I/O enforced on Windows. 72 tests. Scorecard updated: Parity 6→7, OC leads 8→7, NeuralCleave missing 7→6. |
| 2026-07-08 | **OS autostart gap closed** — `AutostartManager` + `neuralcleave autostart enable/disable/status` added (PR #37). Windows: `HKCU\...\Run` registry key via `winreg`. macOS: `~/Library/LaunchAgents/com.neuralcleave.plist` (launchd). Linux: `~/.config/systemd/user/neuralcleave.service`. 91 tests. Scorecard updated: Parity 7→8, OC leads 7→6, NeuralCleave missing 6→5. |
