...
Commands run through `ShellTool(require_approval=True)` are gated by a persistent allowlist + security policy.
...
Security/ask modes (`deny|allowlist|full` and `off|on-miss|always`) are set via `POST /api/v1/approvals/policy` — see [API Reference → Approvals](/api-reference/approvals). Pending approvals are also forwarded as a plain-text message in the channel that triggered them — reply `approve <id-prefix>` or `deny <id-prefix>` directly in chat.
