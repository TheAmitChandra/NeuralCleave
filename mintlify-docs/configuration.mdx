...
The gateway watches `config.toml` and reloads model/API-key settings without a restart. Channel connections are not hot-reloaded — restart the gateway (or use `neuralcleave channels remove`/`add`) after changing channel config.
