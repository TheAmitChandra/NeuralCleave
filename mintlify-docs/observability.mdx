...
```bash
curl http://localhost:8765/api/v1/metrics             # Prometheus text exposition
curl http://localhost:8765/api/v1/metrics/snapshot     # JSON, for the web UI
```
...
Key metrics: `messages_total`/`messages_sent_total`/`messages_errors_total` (by channel), `generation_requests_total`/`generation_errors_total`/`generation_latency_ms` (by model), `tokens_total` and `cost_usd_total` (by model/provider — see [Cost Tracking](#cost-tracking) below), `memory_entries_total`, `approval_decisions_total`, voice metrics (`voice_transcriptions_total`, `voice_synthesis_total`, PTT/VAD/wake-word counters), and host resource gauges (below).
...
```bash
curl http://localhost:8765/health   # always 200 once the process is up
curl http://localhost:8765/ready    # 200 once startup fully completes, 503 while initializing
```
...
```bash
neuralcleave usage
curl http://localhost:8765/api/v1/usage
```
